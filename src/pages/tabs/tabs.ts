...
export class TabsPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public dataProvider: DataProvider, public alertCtrl: AlertController, public modalCtrl: ModalController) {
  }
      ...
      if (conversationsInfo.length > 0) {
        ...
        conversationsInfo.forEach((conversationInfo) => {
          if(conversationInfo.blocked != true){
            this.dataProvider.getConversation(conversationInfo.conversationId).snapshotChanges().subscribe((conversation) => {
              if (conversation.payload.exists()) {
                conversation = { $key: conversation.key, ...conversation.payload.val()};
